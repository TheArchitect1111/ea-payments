...
const registryIds = moduleIdsMatch ? [...moduleIdsMatch[1].matchAll(/'([^']+)'/g)].map((match) => match[1]) : [];
...
assert(inventory.modules.length === registryIds.length, 'inventory must classify every registered module exactly once');
assert(new Set(inventory.modules.map((module) => module.id)).size === inventory.modules.length, 'inventory contains duplicate module ids');
for (const id of registryIds) assert(inventoryById.has(id), `inventory missing registered module ${id}`);
for (const module of inventory.modules) {
  assert(registryIds.includes(module.id), `inventory contains unknown module ${module.id}`);
  assert(['core','business','specialized'].includes(module.class), `${module.id} has invalid class ${module.class}`);
  for (const field of required) assert(Object.hasOwn(module, field), `${module.id} missing contract field ${field}`);
}

const targetClasses = Object.entries(standard.classes).flatMap(([className, value]) => value.target.map((id) => [id, className]));
...
const certifiedCore = inventory.modules.filter((module) => module.class === 'core' && module.assemblyStatus === 'certified');
assert(certifiedCore.length === standard.classes.core.target.length, 'every core target must be certified before universal-core expansion');
for (const id of standard.classes.core.target) {
  ...
  assert(module?.provisioning?.required === true, `${id} must provision automatically with every tenant`);
}

const chassisMatch = registry.match(/CHASSIS_STANDARD_MODULE_IDS: ModuleId\[\] = \[([^\]]+)\]/);
const chassisIds = chassisMatch ? [...chassisMatch[1].matchAll(/'([^']+)'/g)].map((match) => match[1]) : [];
for (const id of standard.classes.core.target) assert(chassisIds.includes(id), `${id} certified core is not enforced by CHASSIS_STANDARD_MODULE_IDS`);

const uncertified = inventory.modules.filter((module) => module.assemblyStatus !== 'certified');
for (const module of uncertified) {
  assert(module.costLicense?.decision !== 'approved' || module.assemblyStatus === 'certified', `${module.id} cannot be silently approved before certification`);
}
