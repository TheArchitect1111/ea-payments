...
export function isUsableResearchUrl(url) {
  ...
  if (/\/api\/ctp\/assets\//i.test(trimmed)) return false;
  if (/^https?:\/\/upload\//i.test(trimmed)) return false;
  return true;
}
...
export function resolveResearchUrl(context) {
  ...
  const fromIntake =
    intake?.normalized?.primaryUrl ||
    intake?.sources?.find((s) => s.type !== 'image' && isUsableResearchUrl(s.url))?.url;
  const candidates = [context.seed?.url, fromIntake];
  for (const candidate of candidates) {
    if (isUsableResearchUrl(candidate)) return candidate.trim();
  }
  return null;
}
