...
import {
  ...
  normalizeLaunchUrl,
} from '../factory-url-normalize.mjs';
...
/** Real website URLs only — never treat CTP launch photo asset links as sites. */
export function isUsableResearchUrl(url) {
  const normalized = normalizeLaunchUrl(url);
  if (!normalized) return false;
  if (/\/api\/ctp\/assets\//i.test(normalized)) return false;
  if (/^https?:\/\/upload\//i.test(normalized)) return false;
  return true;
}
...
export function resolveResearchUrl(context) {
  ...
  const fromIntake =
    normalizeLaunchUrl(intake?.normalized?.primaryUrl) ||
    intake?.sources?.find((s) => s.type !== 'image' && isUsableResearchUrl(s.url))?.url;
  const fromNotes = extractUrlFromLaunchNotes(context.seed?.notes);
  const candidates = [context.seed?.url, fromIntake, fromNotes];
  for (const candidate of candidates) {
    const normalized = normalizeLaunchUrl(candidate);
    if (normalized && isUsableResearchUrl(normalized)) return normalized;
  }
  return null;
}
