const MODULE_RUNTIME = {
  ...
  media: { surface: 'media-library', actions: ['list-media','open-media','upload-media'], bindings: ['files','roles-permissions'] },
  calendar: { surface: 'calendar', actions: ['view-calendar','create-event'], bindings: ['calendar','notifications'] },
  eva: { surface: 'eva', actions: ['chat','update-content','track-opportunities','manage-requests'], bindings: ['messaging','workflow','tenant-data'] }
};
...
export function materializePortalRuntime(workOrder = {}) {
  ...
  const surfaces = modules.map((moduleId) => {
    ...
    return { moduleId, label: nav.label, route: nav.route, surface: runtime.surface, actions: runtime.actions, bindings: runtime.bindings, permissions: permissions[moduleId], dataBindings: dataBindings[moduleId] };
  });
  return {
    kind: 'portal_app', builderId: 'portal-assembly-line', tenantId: workOrder.tenantId, projectId: workOrder.projectId,
    workOrderId: workOrder.id, navigation: navigation.filter((n) => modules.includes(n.moduleId)), surfaces, routes, ctaBindings,
