...
function textBlob(researchArtifacts) {
  ...
  for (const art of researchArtifacts) {
    ...
    if (typeof data.extracted?.textPreview === 'string') parts.push(data.extracted.textPreview);
    if (typeof data.whoTheyAre === 'string') parts.push(data.whoTheyAre);
    if (typeof data.whatTheyDo === 'string') parts.push(data.whatTheyDo);
    if (typeof data.visionSummary === 'string') parts.push(data.visionSummary);
...
export function deriveDiscoveryDrafts(researchArtifacts, options = {}) {
  ...
  const prospect = first(research, 'prospect_profile');
  ...
  const orgIds = idsOf([org, prospect, website, metadata].filter(Boolean));
  const contentIds = idsOf([prospect, website, ...documents, ...branding].filter(Boolean));
  ...
  // 1. Organization Profile
  {
    ...
    const brand = branding.find((b) => b.data?.hasVision || b.data?.visionSummary) || branding[0];
    const extracted = website?.data?.extracted || {};
    const prospectSubject = prospect?.data?.subject || {};
    const prospectIdentity = prospect?.data?.identity || {};
    drafts.push({
      kind: 'organization_profile',
      providerId: 'discovery',
      provenance: baseProvenance(
        [...sourceIds, ...idsOf(branding)].filter(Boolean),
        seedClient,
        intakeOutputId,
        'Derived from research org/website/metadata/branding',
      ),
      data: {
        name:
          prospectSubject.resolvedName ||
          brand?.data?.suggestedClientName ||
          brand?.data?.brandName ||
          org?.data?.organizationName ||
          org?.data?.client ||
          seedClient,
        goal: null,
        // Intake launch goals stay internal — never surface as public profile goal.
        intakeGoal: org?.data?.goal || null,
        deliverable: org?.data?.deliverable || null,
        industry: org?.data?.industry || null,
        primaryUrl: org?.data?.primaryUrl || website?.data?.url || brand?.data?.detectedUrl || null,
        identityStatus: prospectIdentity.status || null,
        identityConfidence: prospectIdentity.confidence ?? null,
        identitySelectedUrl: prospectIdentity.selectedUrl || null,
        citations: Array.isArray(prospect?.data?.citations) ? prospect.data.citations : [],
        publicEvidence: Array.isArray(prospect?.data?.evidence) ? prospect.data.evidence : [],
        assetInventory: Array.isArray(prospect?.data?.assetInventory)
          ? prospect.data.assetInventory
          : [],
        publishingSafety: prospect?.data?.publishingSafety || null,
        websiteTitle: extracted.title || null,
        websiteDescription: extracted.description || null,
        websiteTextPreview: extracted.textPreview || null,
        h1: Array.isArray(extracted.h1) ? extracted.h1 : [],
        navLabels: Array.isArray(extracted.navLabels) ? extracted.navLabels : [],
        ctas: Array.isArray(extracted.ctas) ? extracted.ctas : [],
        entityType: brand?.data?.entityType || null,
        whoTheyAre: brand?.data?.whoTheyAre || brand?.data?.visionSummary || null,
        whatTheyDo: brand?.data?.whatTheyDo || brand?.data?.offer || null,
        audience: brand?.data?.audience || null,
        offer: brand?.data?.offer || brand?.data?.whatTheyDo || null,
        voice: brand?.data?.voice || null,
        primaryAsk: brand?.data?.cta || (Array.isArray(extracted.ctas) ? extracted.ctas[0] : null) || null,
        proof: brand?.data?.proof || null,
        friction: brand?.data?.friction || null,
        opsClue: brand?.data?.opsClue || null,
        opportunities: Array.isArray(brand?.data?.opportunities) ? brand.data.opportunities : [],
  ...
  // 2. Programs
  {
    ...
    if (org?.data?.goal && !/research the subject|matched website|portal experience/i.test(String(org.data.goal))) {
      items.push({
        label: org.data.goal,
  ...
  // 5. Content Inventory
  {
    ...
    if (prospect) {
      for (const citation of prospect.data?.citations || []) {
        entries.push({
          type: 'public_source',
          title: citation.title || citation.url || 'Public source',
          url: citation.url || null,
          sourceType: citation.sourceType || null,
          identityScore: citation.identityScore ?? null,
          sourceArtifactId: prospect.id,
        });
      }
      for (const asset of prospect.data?.assetInventory || []) {
        entries.push({
          type: asset.type || 'public_asset',
          title: asset.network || asset.type || 'Public asset',
          url: asset.url || null,
          sourceUrl: asset.sourceUrl || null,
          usageStatus: asset.usageStatus || 'permission_review_required',
          sourceArtifactId: prospect.id,
        });
      }
    }
