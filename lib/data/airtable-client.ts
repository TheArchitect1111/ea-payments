...
function retryDelayMs(response: Response, attempt: number): number {
  ...
  // Airtable documents a 30-second cooldown after a per-base 429. Respect that
  // floor even when Retry-After is absent so a retry does not immediately fall
  // back into the same rate-limit window. Small jitter prevents synchronized
  // callers from retrying on the exact same millisecond.
