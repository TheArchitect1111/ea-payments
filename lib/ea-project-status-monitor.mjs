...
async function checkPublicPage(project) {
  ...
  try {
    ...
    const missingMarkers = markers.filter((marker) => !text.toLowerCase().includes(String(marker).toLowerCase()));
    if (missingMarkers.length) {
      return state('identity_mismatch', `Page is reachable but required identity markers are missing: ${missingMarkers.join(', ')}.`, { httpStatus: response.status, finalUrl: response.url });
    }
    const finalUrl = response.url || project.publicUrl;
    const allowedFinalHosts = project.allowedFinalHosts ?? [];
    const finalHost = new URL(finalUrl).hostname;
    if (allowedFinalHosts.length && !allowedFinalHosts.includes(finalHost)) {
      return state('unexpected_redirect', `Page redirected to unapproved host ${finalHost}.`, { httpStatus: response.status, finalUrl });
    }

    const discoveredAssets = [...text.matchAll(/<img\b[^>]*\bsrc=["']([^"']+)["']/gi)]
      .map((match) => match[1]
        .replaceAll('&amp;', '&')
        .replaceAll('&#38;', '&')
        .replaceAll('&quot;', '"'))
      .filter((value) => value && !value.startsWith('data:') && !value.startsWith('blob:'));
    const configuredAssets = project.expectedAssets ?? [];
    const assets = [
      ...configuredAssets.map((asset) => typeof asset === 'string' ? { path: asset } : asset),
      ...discoveredAssets.map((path) => ({ path })),
    ];
    const uniqueAssets = [...new Map(assets.map((asset) => [new URL(asset.path, project.publicUrl).href, asset])).entries()]
      .slice(0, 40);
    const assetResults = await Promise.all(uniqueAssets.map(async ([url, requirements]) => {
      try {
        const assetResponse = await fetchWithTimeout(url, {
          redirect: 'follow',
          headers: { 'User-Agent': 'EA-Project-Status/1.0' },
          timeoutMs: 15000,
        });
        const contentType = assetResponse.headers.get('content-type') ?? '';
        const bytes = (await assetResponse.arrayBuffer()).byteLength;
        const typeOk = !requirements.contentType || contentType.toLowerCase().startsWith(String(requirements.contentType).toLowerCase());
        const sizeOk = !requirements.minBytes || bytes >= requirements.minBytes;
        return { url, ok: assetResponse.ok && typeOk && sizeOk && bytes > 0, httpStatus: assetResponse.status, contentType, bytes };
      } catch (error) {
        return { url, ok: false, error: error instanceof Error ? error.message : 'Asset check failed.' };
      }
    }));
    const failedAssets = assetResults.filter((asset) => !asset.ok);
    if (failedAssets.length) {
      return state('asset_failure', `${failedAssets.length} required or referenced image asset(s) failed.`, {
    ...
    return state('healthy', markers.length ? `Page is reachable and all ${markers.length} identity markers are present.` : 'Page is reachable.', {
      httpStatus: response.status,
      finalUrl,
      matchedIdentity: markers,
      checkedAssets: assetResults.length,
    });
  } catch (error) {
...
function overallStatus(project, checks) {
  const critical = new Set(['error', 'protected', 'identity_mismatch', 'unexpected_redirect', 'asset_failure']);
...
export function projectStatusMarkdown(report) {
  ...
  if (critical.length) {
    ...
    for (const project of critical) {
      const details = Object.values(project.checks).filter((check) => ['error', 'protected', 'identity_mismatch', 'unexpected_redirect', 'asset_failure'].includes(check.status)).map((check) => check.detail);
