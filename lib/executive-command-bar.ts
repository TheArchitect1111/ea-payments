...
function workspaceActions(): ExecutiveCommandResult[] {
  const certifiedActions: ExecutiveCommandResult[] = [
    ...
    },
    {
      ...
      name: 'Open Clients',
      type: 'Action',
      summary: 'Open the client delivery board.',
      owner: 'Executive Operating System',
      status: 'Available',
      ...
      action: 'open',
      keywords: ['clients', 'delivery', 'accounts', 'organizations'],
    },
    {
      id: 'open-ctp',
      name: 'Open CTP',
      type: 'Action',
      summary: 'Open CTP submissions and intake desk.',
      owner: 'Executive Operating System',
      status: 'Available',
      source: 'CTP desk',
      confidence: 'High',
      destination: '/admin/ctp',
      action: 'open',
      keywords: ['ctp', 'intake', 'submissions', 'consider'],
    },
    {
      id: 'open-launch-readiness',
      ...
      destination: '/launch',
      action: 'open',
      keywords: ['operations', 'health', 'launch', 'domain', 'ssl', 'api', 'readiness'],
    },
    {
      id: 'create-proposal',
...
export async function buildExecutiveCommandIndex(): Promise<ExecutiveCommandIndex> {
  ...
  const results: ExecutiveCommandResult[] = [
    ...
    ...listExecutiveCapabilities().map((capability): ExecutiveCommandResult => ({
      ...
      destination: '/admin/capability-marketplace',
      action: 'open',
    ...
    ...listKnowledgeAssets().map((asset): ExecutiveCommandResult => ({
      ...
      destination: '/admin/foundation-library',
      action: 'open',
      keywords: normalizeKeywords([
        asset.title,
        asset.category,
  ...
  if (operations) {
    results.push(
      ...operations.platformHealth.map((item): ExecutiveCommandResult => ({
        ...
        destination: '/launch',
        action: 'open',
        keywords: normalizeKeywords([item.label, item.state, item.detail, 'operations', 'health', 'launch']),
      })),
    );
  }
    ...
    results.push(
      ...Object.entries(decisions.queue).flatMap(([horizon, items]) =>
        items.map((item): ExecutiveCommandResult => ({
          ...
          destination: item.href || '/admin/master',
          action: 'review',
          keywords: normalizeKeywords([
            item.title,
            item.reason,
