...
export function derivePlanningBundle(discoveryArtifacts, options = {}) {
  ...
  const storySentence =
    profile?.data?.whoTheyAre ||
    profile?.data?.websiteDescription ||
    profile?.data?.publicEvidence?.[0]?.text ||
    `${orgName} exists to help ${profile?.data?.audience || 'the people it serves'} move forward.`;
  const audience =
    profile?.data?.audience ||
    audiences?.data?.segments?.[0]?.label ||
    'Primary stakeholders and the community served';
  const proofSignals = [
    profile?.data?.proof,
    ...(Array.isArray(profile?.data?.publicEvidence)
      ? profile.data.publicEvidence.map((item) => item?.text)
      : []),
  ...
  drafts.push({
    ...
    data: {
      organizationName: orgName,
      story: {
        sentence: String(storySentence).slice(0, 500),
        audience,
        transformation:
          profile?.data?.goal ||
