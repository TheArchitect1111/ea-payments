...
import { scrubForbiddenPublicCopy } from '../factory-forbidden-copy.mjs';

export { PLANNING_DOCUMENT_KINDS };

function scrubPublic(text) {
  return scrubForbiddenPublicCopy(typeof text === 'string' ? text : undefined);
}
...
export function derivePlanningBundle(discoveryArtifacts, options = {}) {
  ...
  // 8. Creative Direction — public story only (never Factory launch goals)
  const safeEvidence = [
    scrubPublic(profile?.data?.whoTheyAre),
    scrubPublic(profile?.data?.websiteDescription),
    scrubPublic(profile?.data?.whatTheyDo),
    scrubPublic(profile?.data?.websiteTitle),
    ...(Array.isArray(profile?.data?.publicEvidence)
      ? profile.data.publicEvidence.map((item) => scrubPublic(item?.text))
      : []),
  ].filter(Boolean);
  const storySentence =
    safeEvidence[0] ||
    scrubPublic(profile?.data?.offer) ||
    null;
  const audience =
    scrubPublic(profile?.data?.audience) ||
    scrubPublic(audiences?.data?.segments?.[0]?.label) ||
    'People seeking a clear, trusted next step';
  const proofSignals = [
    scrubPublic(profile?.data?.proof),
    ...safeEvidence.slice(1, 6),
  ].filter(Boolean).slice(0, 6);
  const transformation =
    scrubPublic(profile?.data?.whatTheyDo) ||
    scrubPublic(profile?.data?.offer) ||
    (safeEvidence[1] ? String(safeEvidence[1]) : null) ||
  ...
  drafts.push({
    ...
    provenance: baseProvenance(
      idsOf([profile, content, audiences].filter(Boolean)).length
        ? idsOf([profile, content, audiences].filter(Boolean))
        : allIds,
      seedClient,
      intakeOutputId,
      'Evidence-backed visual and narrative direction for website and portal',
    ),
    data: {
      organizationName: orgName,
      story: {
        sentence: storySentence
          ? String(storySentence).slice(0, 500)
          : `${orgName} — a public story grounded in verified evidence.`,
        audience,
        // Never use profile.goal — that field often holds the Factory launch instruction.
        transformation: String(transformation).slice(0, 500),
        proofSignals,
      },
      thinStory: safeEvidence.length < 2,
      experiencePrinciples: [
        'Story before interface',
        'Emotion before features',
        'Identity before industry',
        'The public site and portal must feel like one continuous experience',
      ],
      visualDirection: {
        style: 'editorial, cinematic, human, calm, premium, custom crafted',
        photography: 'documentary realism; real people and environments; natural light',
        typography: 'expressive editorial display type paired with a restrained readable sans',
        composition: 'changing scale, asymmetry, image-led chapters, intentional whitespace',
        motion: 'slow purposeful reveals tied to story progression',
      },
      homepageStoryBeats: [
        'Who they are',
        'Why they exist',
        ...
        'Only then: offers and next step',
      ],
      portalContinuity: {
        purpose: 'Continue the public story inside a calm executive workspace',
        firstView: ['where I am', 'what happened', 'what happens next', 'what needs attention'],
      },
      antiPatterns: [
        'generic card grids',
        'corporate SaaS dashboard styling',
        'repeated equal-height feature sections',
        'corporate blue gradients',
        'generic AI illustration',
        'designs identifiable only by their logo',
      ],
      sourceAssetInventory: Array.isArray(profile?.data?.assetInventory)
        ? profile.data.assetInventory
        : [],
      publishingSafety: profile?.data?.publishingSafety || null,
    },
  });
  ...
  const workOrderSpecs = [
    {
      ...
      acceptanceCriteria: [
        ...
        'Creative direction is represented in story, composition, typography, and imagery',
        'Public site and portal use one continuous visual language',
      ],
