/**
 * Launch health diagnostic payload (operator / CI only).
 * Public HTTP surface must never return this object — see /api/health/launch.
 */
...
export type LaunchHealthPublicSummary = {
  ok: boolean;
  status: string;
};

export async function buildLaunchHealthDiagnostic(): Promise<{
  summary: LaunchHealthPublicSummary;
  ...
  const summary: LaunchHealthPublicSummary = {
