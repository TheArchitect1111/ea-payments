...
const courses = [
  ...
  ['Body Sculpt Practitioner Certification', '$2,497 CAD'],
  ['Non-Surgical BBL Training', '$1,497 CAD'],
  ['Tummy Tuck Sculpt + Injection Integration', '$2,497 CAD'],
] as const;
...
export default function AmandaOwnerDashboard() {
  return <div className="ac-dashboard">
    ...
    <section className="ac-hero">
      <div><span className="ac-kicker">AESTHETIKINE STUDIO LAB</span><h2>Make space for the work that matters.</h2><p>Appointments, clients, education, LIFELINE and business intelligence are gathered here, with Eva ready to guide the next move.</p></div>
      <div className="ac-hero-art" aria-label="Wellness studio visual"><span>body · mind · business</span></div>
    </section>
    ...
    <section className="ac-card ac-programs"><div className="ac-section-title"><div><span>AESTHETIKINE ACADEMY</span><h2>Programs & Courses</h2></div><Link href="/portal/amanda-catherine/owner/academy">Manage Academy →</Link></div><div className="ac-course-list">{courses.map(([name, price]) => <div key={name}><strong>{name}</strong><span>{price}</span></div>)}</div></section>
