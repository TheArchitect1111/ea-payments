...
export default async function AmandaOwnerLayout({children}:{children:ReactNode}){const cookieStore=await cookies();const isClientTest=process.env.VERCEL_ENV==='preview'&&process.env.VERCEL_GIT_COMMIT_REF==='client-test/amanda-v2-20260919'&&cookieStore.get(AMANDA_CLIENT_TEST_COOKIE)?.value===AMANDA_CLIENT_TEST_TOKEN;const token=cookieStore.get(EA_PORTAL_COOKIE)?.value;const session=token?await verifySession(token):null;if(!isClientTest&&(!session||session.slug!=='amanda-catherine'))redirect('/portal/login?next=%2Fportal%2Famanda-catherine%2Fowner');if(!isClientTest&&!roleAtLeast(normalizeRole(session?.role),'staff'))redirect('/portal/amanda-catherine');return <div className="ac-owner-shell"><aside className="ac-sidebar"><div className="ac-brand"><span>AK</span><div><strong>AesthetiKine</strong><small>Studio Lab</small></div></div><nav>{nav.map(([label,href])=><Link key={href} href={href}>{label}</Link>)}</nav><div className="ac-owner-card"><div className="ac-avatar">AC</div><div><strong>Amanda Catherine</strong><small>Owner · Studio Director</small></div></div></aside><main className="ac-main">{children}</main></div>}
