          ...
          {displayed.map((request) => {
            ...
            return (
              <article key={request.id} className="border border-neutral-200 bg-white p-5">
                  ...
                  <div>
                    ...
                    {social && request.videoLink ? (
                      <p className="mt-2 text-xs text-neutral-500">
                        Story:{' '}
                        <a href={request.videoLink} className="underline text-[#1B2B4D]" target="_blank" rel="noreferrer">
                          {request.videoLink}
                        </a>
                      </p>
                    ) : null}
                ...
                <div className="mt-4 grid gap-4 lg:grid-cols-2">
                  ...
                  <div>
                    <p className="text-xs font-bold uppercase tracking-wider text-neutral-400">
                      {social ? 'Publish caption (social + feed)' : 'Publish body (client feed)'}
                    </p>
                    <textarea
                ...
                {publishNotes[request.id] ? (
                  <p className="mt-3 text-sm font-semibold text-green-800">{publishNotes[request.id]}</p>
                ) : null}
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  >
                    {social ? 'Approve & publish social' : 'Publish now'}
                  </button>
                  <button
                    type="button"
                    onClick={() =>
