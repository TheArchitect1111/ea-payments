...
const config: Config<StudioProps> = {
  components: {
    EAHero: {
      label: 'Hero',
      fields: {
        eyebrow: { type: 'text' }, title: { type: 'text' }, description: { type: 'textarea' }, buttonLabel: { type: 'text' },
      },
      defaultProps: {
        eyebrow: 'Efficiency Architects',
        title: 'Build an experience people understand immediately.',
        description: 'Shape the visual experience here while the EA Chassis remains responsible for the operational infrastructure underneath it.',
    ...
    EAFeatures: {
      label: 'Features',
      fields: { title: { type: 'text' }, items: { type: 'textarea' } },
      defaultProps: { title: 'What this experience includes', items: 'Clear customer journey\nConnected portal\nAutomated next steps' },
      render: ({ title, items }) => (
        <section style={{ padding: '64px 7vw' }}><div style={{ maxWidth: 980, margin: '0 auto' }}>
    ...
    EAGallery: {
      ...
      render: ({ title, imageUrl, alt }) => (
        <section style={{ padding: '64px 7vw' }}><div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 style={{ fontSize: 42, letterSpacing: '-.04em' }}>{title}</h2>
    ...
    EAFAQ: {
      ...
      defaultProps: { question: 'What powers this website?', answer: 'Design Studio assembles the visual experience. EA Chassis services remain responsible for the operational functions behind it.' },
      render: ({ question, answer }) => (
        <section style={{ padding: '40px 7vw 72px' }}><div style={{ maxWidth: 860, margin: '0 auto', borderTop: '1px solid #ddd', paddingTop: 28 }}>
          <h3 style={{ fontSize: 26 }}>{question}</h3><p style={{ fontSize: 18, lineHeight: 1.6, color: '#5f5f5f' }}>{answer}</p>
        </div></section>
      ),
    },
  },
...
const initialData: Data = { content: [], root: { props: { title: 'Untitled EA Website' } } };
...
export default function WebsiteBuilderClient() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f5f5f2' }}>
      <header style={{ padding: '20px 28px 18px', background: '#111', color: '#fff', borderBottom: '1px solid #2c2c2c' }}>
        <nav style={{ display: 'flex', justifyContent: 'space-between', gap: 18, alignItems: 'center', flexWrap: 'wrap' }}>
          <Link href="/studio" style={{ color: '#fff', textDecoration: 'none', fontSize: 12, textTransform: 'uppercase', letterSpacing: '.16em', fontWeight: 700 }}>Efficiency Architects · Design Studio</Link>
          <Link href="/studio/create" style={{ color: '#bbb', textDecoration: 'none', fontSize: 13 }}>Studio Projects</Link>
        </nav>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 24, alignItems: 'end', marginTop: 22, flexWrap: 'wrap' }}>
          <div>
            <p style={{ margin: 0, fontSize: 11, textTransform: 'uppercase', letterSpacing: '.18em', color: '#d2b56c', fontWeight: 800 }}>Website · Visual Builder</p>
          ...
          <div style={{ fontSize: 12, color: '#aaa', textAlign: 'right' }}>
            <div>{approvedBlockCount} Website Engine blocks registered</div><div>Chassis registry connected · {status}</div>
          </div>
        </div>
      </header>
