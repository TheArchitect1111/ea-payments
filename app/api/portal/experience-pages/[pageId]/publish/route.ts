...
export async function POST(req: NextRequest, { params }: { params: Promise<{ pageId: string }> }) {
  ...
  if (!result.ok) return NextResponse.json({ ok: false, error: result.detail }, { status: 400 });
  return NextResponse.json({ ok: true, detail: result.detail, href: result.href, mode: result.mode, page: result.page });
}
