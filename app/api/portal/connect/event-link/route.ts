...
export async function POST(req: NextRequest) {
  ...
  const campaignId = `${auth.session.slug}-${event.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 24)}${rep ? `-${rep.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 16)}` : ''}`;
  const label = rep ? `${rep} — ${event}` : event;
  const params = new URLSearchParams();
  params.set('event', event);
  if (rep) params.set('rep', rep);
  params.set('campaign', campaignId);
  const destination = `/connect/${auth.session.slug}?${params.toString()}`;
  ...
  try {
    const result = await addConnectCampaign({ orgSlug: auth.session.slug, campaign });
    const kit = buildConnectKit(result.org, auth.session.slug);
    const url = buildEventCaptureUrl(auth.session.slug, { event, rep, campaignId });
