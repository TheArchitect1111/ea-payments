...
export async function POST(req: NextRequest) {
  ...
  const loginUrl = new URL('/simplifi/login', req.nextUrl.origin);
  ...
  if (password !== confirm) {
    const url = new URL('/simplifi/reset-password', req.nextUrl.origin);
  ...
  try {
    ...
    console.error('Simplifi password reset failed:', err);
    const url = new URL('/simplifi/reset-password', req.nextUrl.origin);
    url.searchParams.set('recordId', recordId);
    url.searchParams.set('token', token);
