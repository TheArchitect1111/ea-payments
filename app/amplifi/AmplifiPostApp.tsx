...
type CampaignPost = { title: string; caption: string; callToAction: string; imageDirection: string; imageUrl?: string; productId?: string; audienceId?: string; waveId?: string };
...
export default function AmplifiPostApp({
  ...
  useEffect(() => {
    ...
    else if (status.endsWith('-instagram-missing')) setConnectionsError('Facebook connected successfully. Meta did not return an Instagram professional account for either selected Page. Connect the Instagram account to the correct Facebook Page in Meta, then reconnect.');
          ...
          <section className="af-primary-column">
            ...
            {selectedPath === 'smartchitecture' ? <section className="af-panel af-smartchitecture-panel" id="smartchitecture">
              ...
              <div className="af-editor-grid">
                ...
                <label className="af-field"><span>What do you want people to understand or do?</span><input value={campaignResult} onChange={(e) => setCampaignResult(e.target.value)} placeholder="Example: See where their business is losing time" /></label>
                <label className="af-field"><span>How should the posts sound?</span><select value={campaignTone} onChange={(e) => setCampaignTone(e.target.value as CampaignTone)}><option value="Bold and direct">Clear and confident</option><option value="Provocative and challenging">Bold and attention-grabbing</option><option value="Authoritative and premium">Professional and polished</option><option value="Warm and human">Warm and conversational</option></select></label>
                <label className="af-field af-field-wide"><span>What real example or result can Amplifi mention? <small>optional</small></span><input value={campaignProofPoint} onChange={(e) => setCampaignProofPoint(e.target.value)} placeholder="Example: We saved one client $17,000 by automating one process" /></label>
                <label className="af-field af-field-wide"><span>What problem are these people dealing with? <small>optional</small></span><input value={campaignPainQuestion} onChange={(e) => setCampaignPainQuestion(e.target.value)} placeholder="Example: Too much time is being lost to repeated manual work" /></label>
                {promotionScope === 'single' ? <><label className="af-field"><span>What should people do next?</span><input value={campaignCallToAction} onChange={(e) => setCampaignCallToAction(e.target.value)} placeholder="Example: Take the free CTP" /></label>
                <label className="af-field"><span>Where should the link take them?</span><input type="url" value={campaignCtaUrl} onChange={(e) => setCampaignCtaUrl(e.target.value)} placeholder="https://cc.efficiencyarchitects.online/ctp" /></label></> : null}
              </div>
              ...
              <button type="button" className="af-more-control" aria-expanded={showCampaignControls} onClick={() => setShowCampaignControls((value) => !value)}>Fine-tune the campaign <span>{showCampaignControls ? '−' : '+'}</span></button>
              {showCampaignControls ? <div className="af-editor-grid af-advanced-controls">
                <label className="af-field"><span>How strong should the message feel?</span><select value={campaignToneStrength} onChange={(e) => setCampaignToneStrength(e.target.value)}><option value="Measured">Calm</option><option value="Balanced">Balanced</option><option value="Strong">Strong</option></select></label>
                <label className="af-field"><span>What should the images look like?</span><select value={campaignImageStyle} onChange={(e) => setCampaignImageStyle(e.target.value)}><option>Branded result graphics</option><option>Bold text graphics</option><option>Clean photography</option><option>Simple editorial design</option></select></label>
                <label className="af-field"><span>Are there words you want Amplifi to use?</span><input value={campaignWordsUse} onChange={(e) => setCampaignWordsUse(e.target.value)} placeholder="Example: time, control, measurable" /></label>
                <label className="af-field"><span>Are there words Amplifi should avoid?</span><input value={campaignWordsAvoid} onChange={(e) => setCampaignWordsAvoid(e.target.value)} placeholder="Example: revolutionary, game-changing" /></label>
