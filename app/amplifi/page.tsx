...
export default async function AmplifiMarketingPage({ searchParams }: { searchParams: Promise<{ url?: string; title?: string; capture?: string }> }) {
  ...
  return <main className="amp-page">
    ...
    <section className="amp-campaign-proof" id="how">
      <header><p className="amp-kicker">ONE EVENT. A COMPLETE SET OF POSTS.</p><h2>Tell Amplifi about<br/>the event once.</h2><p>Amplifi prepares the announcement, reminders, platform versions, day-of posts, thank-you message, and recap.</p></header>
      <div className="amp-campaign-board">
        ...
        <div className="amp-post-stream"><div className="amp-stream-head"><div><span>AMPLIFI CAMPAIGN</span><h3>9 posts prepared</h3></div><p>Every post is written for its platform and placed in the campaign sequence.</p></div><div className="amp-platform-posts">{[
          ['Facebook','Event announcement','Ready for review'],['Instagram','Square announcement','Draft'],['LinkedIn','Partner and sponsor post','Approved'],['Facebook','Registration reminder','Scheduled'],['Instagram','Activity spotlight','Ready for review'],['LinkedIn','Sponsor recognition','Approved'],['Facebook','Deadline reminder','Scheduled'],['Instagram','Day-of event post','Draft'],['LinkedIn','Thank-you and results','Planned']
        ].map(([platform,title,status],i)=><article key={title} className={`platform-${platform.toLowerCase()}`}><div><b>{platform}</b><small>{status}</small></div><span>{String(i+1).padStart(2,'0')}</span><h4>{title}</h4><p>{i < 3 ? 'Introduce the event to the right audience.' : i < 6 ? 'Keep interest and registration moving.' : 'Carry the story through the event and beyond.'}</p></article>)}</div></div>
      </div>
    </section>
    ...
    <section className="amp-human-story"><div className="amp-human-image"><Image src="/amplifi/amplifi-person-phone-v2.jpg" alt="A business leader reviewing Amplifi from a phone" width={1672} height={941} sizes="(max-width: 900px) 100vw, 52vw"/></div><div className="amp-human-copy"><p className="amp-kicker">CONTROL WITHOUT THE DESK</p><h2>Review the work.<br/>Keep your day.</h2><p>Approve a post, request a change, adjust the schedule, or check campaign progress from your phone. Amplifi prepares the work. You remain the decision-maker.</p><blockquote>Nothing publishes outside the control level you choose.</blockquote></div></section>
