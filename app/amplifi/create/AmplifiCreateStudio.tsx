...
export default function AmplifiCreateStudio({initialMode='campaign'}:{initialMode?:Mode}) {
  const [mode,setMode]=useState<Mode>(initialMode); const [brandName,setBrandName]=useState(''); const [websiteUrl,setWebsiteUrl]=useState(''); const [topic,setTopic]=useState(''); const [goal,setGoal]=useState(''); const [goalOther,setGoalOther]=useState(''); const [audience,setAudience]=useState(''); const [offer,setOffer]=useState(''); const [cta,setCta]=useState(''); const [destinationUrl,setDestinationUrl]=useState(''); const [timing,setTiming]=useState(''); const [details,setDetails]=useState(''); const [count,setCount]=useState(7); const [busy,setBusy]=useState(false); const [error,setError]=useState(''); const [creative,setCreative]=useState<GeneratedItem[]>([]); const [campaign,setCampaign]=useState<Campaign|null>(null); const [premium,setPremium]=useState<PremiumPackage|null>(null);
  ...
  return <main className="af-create-shell">
    ...
    <section className="af-create-intro"><small>GIVE AMPLIFI THE IDEA</small><h1>What would you like to create?</h1><p>Choose the job. Amplifi will ask only what it needs, then the Creative Foundry produces the actual work.</p></section>
