...
test('amplifi landing page is reachable', async ({ page }) => {
  ...
  await expect(page.getByText(/^SMARTCHITECTURE$/i).first()).toBeVisible();
  await expect(page.getByRole('heading', { name: /your campaign control room has a guide/i })).toBeVisible();
